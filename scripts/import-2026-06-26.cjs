/**
 * MarCRM Import Script — 2026-06-26 (Prisma)
 * --------------------------------------------------------------
 * Daily scrape: 7 new leads
 *   pr-marketing (5): Caplight Technologies, Corgi Insurance,
 *                     Scaled Cognition, Hang Ten Systems, Capchase
 *   pr-freelance (1): Seed-Series A VC Fund (via Sapio Consulting)
 *   fund-formation (1): Star51 Capital
 *
 ...
 *   node scripts/import-2026-06-26.cjs            # insert missing leads
 *   node scripts/import-2026-06-26.cjs --dry-run  # print plan, write nothing
 *
...
const REPORT = path.join(__dirname, '..', 'reports', 'daily-scrape-2026-06-26.json');
...
async function main() {
  ...
  const leads = (report.newLeads || []).map(toCompanyData);
